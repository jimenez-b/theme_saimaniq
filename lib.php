...
function theme_saimaniq_get_main_scss_content($theme) {        
    ...
    if ($filename == 'default.scss' || $filename == 'plain.scss') {
        // We still load the default preset files directly from the boost theme. No sense in duplicating them.    
        $scss .= file_get_contents($CFG->dirroot . '/theme/boost/scss/preset/'.$filename);
    } else if ($filename == 'saimaniq.scss' || $filename == 'cole.scss') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/saimaniq/scss/preset/'.$filename);
